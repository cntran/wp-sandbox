...
register_widget( 'sdbx_widget_tags' );
class sdbx_widget_tags extends WP_Widget {
	 ...
	 * @since 0.6
	 */
	function sdbx_widget_tags() {
		$this->prefix = 'sdbx';
		$this->textdomain = 'sdbx';
		...
		$this->WP_Widget( "{$this->prefix}-tags", __( 'SDBX Tags', $this->textdomain ), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( 'flat' == $instance['format'] ) {
			...
			for ( $i = 0;  $i < count($instance['exclude']); $i++ ) {
				$terms_excluded .= $instance['exclude'][$i];
				if ( $i !== count($instance['exclude']) - 1 )
					$terms_excluded .= ',';
			}
			
			global $wpdb;
		
			$querystr = "SELECT t.term_id, t.name, tt.count
						 FROM wp_term_taxonomy tt
						 JOIN wp_terms t ON tt.term_id = t.term_id
						 WHERE tt.taxonomy = 'post_tag' ";
			
			if ( '' !== $terms_excluded )
				$querystr .= " AND t.term_id NOT IN (" . $terms_excluded . ") ";
			
			if ( '' !== $args['name__like'] ) {
				$querystr .= " AND t.name LIKE '%" . $args['name__like'] . "%' ";
			}
			
			$querystr .= "ORDER by tt.count DESC;";
								
			$terms = $wpdb->get_results($querystr, OBJECT);
			$tags = '<ul>';				
			foreach ( $terms as $term ) {
				$tags .= '<li><a href="' . get_tag_link($term->term_id) . '">' . $term->name . ' (' . $term->count . ')</a></li>';
			}
			$tags .= '</ul>';
		...
		if ( function_exists(sdbx_tag_cloud) )
			$tags = sdbx_tag_cloud();
	...
	function form( $instance ) {
		...
		<div class="sdbx-widget-controls columns-3">
		<p>
		...
		<div class="sdbx-widget-controls columns-3 column-last">
		<p>
