...
class sdbx_post_thumbnails {
	...
	public function save_meta_box( $post_id ) { 
		...
		if ( $this->id !== "_sdbx_featured_slide" )
			return;
		
		if ( isset( $_POST["{$this->id}_order"] ) ) {
			update_post_meta( $post_id, "{$this->id}_order", esc_attr($_POST["{$this->id}_order"]) );
		}
	...
	public function thumbnail_meta_box() {
		global $post;
		$thumbnail_id = get_post_meta($post->ID, "{$this->id}_thumbnail_id", true);
		...
		$sdbx_thumbnail_order = get_post_meta($post->ID, "{$this->id}_order", true);
		if ( '' === $sdbx_thumbnail_order )
			$sdbx_thumbnail_order = 0;
			 
		echo "<p><label for='{$this->id}_order'><strong>Order</strong></label></p><input id='{$this->id}_order' name='{$this->id}_order' type='text' value='$sdbx_thumbnail_order' size='4' />";
	...
	public function add_attachment_field($form_fields, $post) {
		...
		$link = sprintf('<a id="%4$s-%1$s-thumbnail-%2$s" class="%1$s-thumbnail" href="#" onclick="PostThumbnailsSetAsThumbnail(\'%2$s\', \'%1$s\', \'%4$s\', \'%5$s\');return false;">Set as %3$s</a>', $this->id, $post->ID, $this->label, $this->post_type, $ajax_nonce);
	...
	public function enqueue_admin_scripts() {
		...
		wp_enqueue_script( "sdbx-custom-featured-image",  THEME_URI  . '/library/extensions/post-thumbnails/post-thumbnails.js', array('jquery'));
	}
	...
	private function post_thumbnail_html($thumbnail_id = NULL) {
		...
		if ($thumbnail_id && get_post($thumbnail_id)) {
			...
			if (!empty($thumbnail_html)) {
				...
				$content .= sprintf('<p class="hide-if-no-js"><a href="#" id="remove-%1$s-%2$s-thumbnail" onclick="PostThumbnailsRemoveThumbnail(\'%2$s\', \'%1$s\', \'%4$s\');return false;">%3$s</a></p>', $this->post_type, $this->id, esc_html__( "Remove {$this->label}" ), $ajax_nonce);
			}
	...
	public function set_thumbnail() {
		...
		if ($thumbnail_id == '-1') {
			delete_post_meta($post_ID, "{$this->id}_thumbnail_id");
		...
		if ($thumbnail_id && get_post($thumbnail_id)) {
			...
			if (!empty($thumbnail_html)) {
				update_post_meta( $post_ID, "{$this->id}_thumbnail_id", $thumbnail_id );
