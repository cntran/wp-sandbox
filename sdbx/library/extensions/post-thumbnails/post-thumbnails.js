function PostThumbnailsSetThumbnailHTML(html, id, post_type){
	jQuery('#' + id, '#' + post_type + '-' + id).html(html);
};

function PostThumbnailsSetThumbnailID(thumb_id, id, post_type){
...
function PostThumbnailsRemoveThumbnail(id, post_type, nonce){
	jQuery.post(ajaxurl, {
		...
		if ( str == '0' ) {
			...
			PostThumbnailsSetThumbnailHTML(str, id, post_type);
		}
	}
	);
};
...
function PostThumbnailsSetAsThumbnail(thumb_id, id, post_type, nonce){
	...
	jQuery.post(ajaxurl, {
		...
		if ( str == '0' ) {
			...
			win.PostThumbnailsSetThumbnailID(thumb_id, id, post_type);
			win.PostThumbnailsSetThumbnailHTML(str, id, post_type);
		}
	}
	);
}